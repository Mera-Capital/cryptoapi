...
from .url import UsdmURL
from .mapping import _USDM_MAPPER


class BinanceUsdm(BinanceClient, ExchangeInterface):
    def __init__(self, testnet: bool, client: HTTPClientInterface):
        super().__init__(client)
        self._url = UsdmURL(testnet)
        self._mapper = _USDM_MAPPER

    async def get_instruments(self) -> list[Instrument]:
        raw_result, instruments = await self.get(self._url.instruments), []
        return [self._mapper.load(
            {
                "section": Section.USDM,
                "kind": Kind.FUTURE,
                "active_status": True,
